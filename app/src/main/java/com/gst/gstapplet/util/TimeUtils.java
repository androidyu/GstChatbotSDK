...
public class TimeUtils {
    public static String value(Date oldDate, Date newDate) throws ParseException {
        // TODO Auto-generated method stub
        String dateline = null;
        SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String newTime = sdf1.format(newDate);
        String oldTime = sdf1.format(oldDate);
//        Date d1 = sdf1.parse(oldDate);
//        Date d2 = sdf1.parse(newDate);
        dateline = String.valueOf(daysBetween(oldDate, newDate)) + "天前"; //获取两个时间的天数差
        if (daysBetween(oldDate, newDate) == 0) {     //判定是否是同一天，不是一天的话进行下一步判断
            //判定一天内的间隔时间
            dateline = getTimeGap(oldTime.substring(oldTime.length() - 8), newTime.substring(newTime.length() - 8));
        } else if (daysBetween(oldDate, newDate) == 1) {  //相差一天，就返回昨天
            dateline = "昨天";
        }
